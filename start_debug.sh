...
echo "[DEBUG] Compilando..."
make debug || { echo "Falha na compilacao!"; exit 1; }

echo "[DEBUG] Iniciando display virtual..."
Xvfb $DISPLAY_NUM -screen 0 1280x720x24 2>/dev/null &
sleep 2

echo "[DEBUG] Iniciando VNC..."
x11vnc -display $DISPLAY_NUM -rfbport $VNC_PORT -forever -nopw -shared -quiet 2>/dev/null &
sleep 1

echo "[DEBUG] Iniciando noVNC na porta $WEB_PORT..."
novnc --listen 0.0.0.0:$WEB_PORT --vnc localhost:$VNC_PORT --web $NOVNC_WEB 2>/dev/null &
sleep 2

echo "[DEBUG] Iniciando jogo (modo DEBUG)..."
export DISPLAY=$DISPLAY_NUM
$BINARY &
GAME_PID=$!

echo "[DEBUG] Jogo rodando! Acesse: porta $WEB_PORT -> /vnc_lite.html?autoconnect=true"
wait $GAME_PID
echo "[DEBUG] Jogo encerrado."
